...
def simpleTokenize(string):
    ...
    """
#     return <FILL IN>
    match = filter(None, re.split(split_regex, string.lower()))
    return match
...
def findBiggestRecord(vendorRDD):
    ...
    """
#     return <FILL IN>
#     return vendorRDD.flatMap(lambda (a,b): (b, len(b))).collect()
...
def tf(tokens):
    ...
    newDict = dict((i, float(tokens.count(i))) for i in tokens)
    new_dict = {k:(v/sum(newDict.values())) for k, v in newDict.items()}        
...
#     return (tokenSumPairTuple.<FILL IN>)
    ...
    tokenCountPairTuple = uniqueTokens.map(lambda (x,y): (y, 1))
...
def norm(a):
    ...
    """
#     return <FILL IN>    
    return math.sqrt(sum({k:(v**2) for k, v in a.items()}.values()))
...
#     googleURL = googleRec.map(lambda (x,y): x)
#     amazonID = amazonRec.map(lambda (x,y): y)
...
trueDupsCount = trueDupsRDD.map(lambda (a,b): (a,1)).reduceByKey(lambda a, b : a + b).count()
# print trueDupsCount
...
avgSimDups = (trueDupsRDD
              .map(lambda (ID,(cosScore,goldStr)):(ID,cosScore))
              .reduceByKey(lambda x,y : x+y)
              .map(lambda (ID,score):score)
              .sum()/float(trueDupsCount))
# print avgSimDups
...
# amazonNorms = amazonWeightsRDD.map(lambda (a,b): (a, norm(b))).collect()
# amazonNormsBroadcast = sc.broadcast(amazonNorms)
# googleNorms = googleWeightsRDD.map(lambda (a,b): (a, norm(b))).collect()
...
#     total=0
#     for item in tokens:
#         total+= googleWeightsBroadcast.value[googleRec][item] * amazonWeightsBroadcast.value[amazonRec][item]
#     value = total /(amazonNormsBroadcast.value[amazonRec])
#     value = value / googleNormsBroadcast.value[googleRec]    
#     key = (amazonRec, googleRec)
#     return (key, value)
